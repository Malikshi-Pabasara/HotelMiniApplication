...
namespace Cenium.Reservations.Client.Windows.Actions
    {
        ...
        /// </summary>
        /// 
        [RegisterActionType("reservations.cancelaction")]
        public class CancelAction : AbstractActionHandler
        {
            ...
            protected override bool EvaluateConditions()
            {
                ...
                if (test)
                {
                    if (ClientConnector.Current.CheckUserAccess("Reservations/Reservation/Cancel"))
                    {
                        return true;
                    }
                }
            ...
            protected override void OnExecute()
            {
                ...
                {
                //Make sure we have a contact before allowing cancel
                long ContactId = 0;
                    try
                    {
                    ...
                    if (ContactId == 0L)
                    {
                        MessageBox.Show("You have to add/set a contact before cancel a reservation.");
                        return;
                    }
                    else
                    {
                        MessageBoxResult result = MessageBox.Show("Do you want to cancel the reservation?",
                       "cancel", MessageBoxButton.YesNo);
                        if (result == MessageBoxResult.Yes)
                        {
                            WindowManager.ShowPageProgress(Owner, "Cancel Reservation", "Cancel Reservation...");

                            try
                            {
                                ClientConnector.Current.InvokeAsync(new ServiceInvokeParameters
                                {
                                    ServiceMethod = "Reservations/Reservation/Cancel",
                                    Record = rec,
                            ...
                            catch (Exception ex)
                            {
                                ...
                                MessageBox.Show(string.Format("{0}\n{1}\n{2}", "Error occured when cancel reservation.",
            ...
            private void Finished(ServiceOperationResult result)
            {
                ...
                if (!result.IsError)
                {
                    ...
                    EventDispatchManager.ExecuteOnUIThread(
                        (Action)delegate ()
                        {
                            MessageBox.Show("Reservation has been set to canceled", "Cancel", MessageBoxButton.OK, MessageBoxImage.Information);
                        });
                }
                else
                {
